...
static void root_key_event_cb(lv_event_t* e)
{
    ...
    {
        if (focused == get_list_back_button(binding) && is_visible(focused))
        {
            set_column(binding, FocusColumn::Filter);
            rebind_by_column(binding);
            return;
...
void init(Binding* binding, const Adapter& adapter)
{
    ...
    if (lv_obj_t* key_target = adapter.get_key_target ? adapter.get_key_target(adapter.ctx) : nullptr)
    {
